...
public abstract class ChiseledTuffBricksMixin {
    ...
    @Definition(id = "Block", type = net.minecraft.world.level.block.Block.class)
    @Expression("register('chiseled_tuff_bricks', @(new Block(?)))")
    @WrapOperation(method = "<clinit>", at = @At("MIXINEXTRAS:EXPRESSION"))
    private static Block makeChiseledTuffBricksPillarBlock(BlockBehaviour.Properties properties, Operation<Block> original) {
